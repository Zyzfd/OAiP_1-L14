...
void addWord(struct Dictionary* dict, char* word)
{
	...
	newNode->next = dict->first[hash_value];
	newNode->word = (char*)calloc(strlen(word) + 1,
		sizeof(char));
	strcpy(newNode->word, word);

	dict->cnt_words++;
	dict->first[hash_value] = newNode;
}
...
void destroy(struct Dictionary* dict) {
	...
	for (int i = 0; i < MAX_HASH; i++)
		...
		while (dict->first[i] != NULL)
		{
			struct Node* delNode = dict->first[i];
			dict->first[i] = dict->first[i]->next;
...
int contains(struct Dictionary* dict, char* word)
{
	...
	struct Node* ptr = dict->first[hash_value];
	while (ptr != NULL) {
